...
const PrintedcardForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if(name === 'cardWeight' || 'numberCards'){
      // Convert value to a number
    const numericValue = Number(value);
    // Validate the input for this specific field
    if(name==='cardWeight' && numericValue > 480){
      setErrors((prevErrors) => ({
        ...prevErrors,
        [name]: "Maximum allowed value is 450.",
      }));
      // setTimeout(()=> {
      //   setErrors((prevErrors) => ({
      //     ...prevErrors,
      ...
      setFormData({
        ...formData,
        [name]: 450, // Set to 10 crore if input exceeds the limit
      });
    }
    else if (name === 'numberCards' && numericValue > 100000000) {
      setErrors((prevErrors) => ({
        ...prevErrors,
        [name]: "Maximum allowed value is 100000000.",
      }));
      setTimeout(()=> {
        setErrors((prevErrors) => ({
          ...prevErrors,
      ...
      setFormData({
        ...formData,
        [name]: 100000000, // Set to 10 crore if input exceeds the limit
      });
    }else {
      setErrors((prevErrors) => ({
        ...prevErrors,
        [name]: "", // Clear error for this field if within limit
      }));
      setFormData({
        ...formData,
        [name]: numericValue,
        ...
        <form className="" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            <div className="mb-6">
              ...
              />
              {errors['cardWeight'] && (
                <p className="mt-2 text-red-600 outfit-medium">
                  {errors['cardWeight']}
                </p>
              )}
            </div>
