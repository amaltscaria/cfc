...
const Certificate = React.forwardRef((props, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="w-[340px] h-[528.5px] md:w-[420.5px] md:h-[630px] border-[14px] border-[#40A578] relative "
    >
      ...
      <div className="flex justify-center h-[158.5px] md:h-[215px]">
        <img src={footer} alt="" />
      </div>
    </div>
  );
...
const CarbonZeroDay = ({ emission }) => {
  ...
  const captureElement = (element, scale = 2) => {
    return new Promise((resolve) => {
      ...
      bgDiv.style.backgroundColor = "#40A578";
  ...
  const downloadPDF = () => {
    ...
    html2canvas(input, { scale: scale }).then((canvas) => {
      ...
      const imgX = (pdfWidth - imgWidth * ratio) / 2 + 0.3;
